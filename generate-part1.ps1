...
try {
    ...
    foreach ($s in $students) {
        $sheet.Cells.Item($row, 1).NumberFormat = "@"
        $sheet.Cells.Item($row, 1).Value2 = [string]$no
        
        $sheet.Cells.Item($row, 2).NumberFormat = "@"
        $sheet.Cells.Item($row, 2).Value2 = [string]$s.C4
        
        $sheet.Cells.Item($row, 4).NumberFormat = "@"
        $sheet.Cells.Item($row, 4).Value2 = [string]$s.C2
        
        $sheet.Cells.Item($row, 9).NumberFormat = "@"
        $sheet.Cells.Item($row, 9).Value2 = [string]$s.Jenjang
